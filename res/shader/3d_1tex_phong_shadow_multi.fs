...
uniform sampler2D texture_shadow0;
...
uniform sampler2D texture_shadow7;
uniform sampler2D texture_shadow8;
uniform sampler2D texture_shadow9;
...
float isInShadowPCF(vec4 fragPosLightSpace,int texIndex){
    ...
    if(texIndex==0)
        texelSize=1.0/textureSize(texture_shadow0,0);
    else if(texIndex==1)
        texelSize=1.0/textureSize(texture_shadow1,0);
    else if(texIndex==2)
        texelSize=1.0/textureSize(texture_shadow2,0);
    else if(texIndex==3)
        texelSize=1.0/textureSize(texture_shadow3,0);
    else if(texIndex==4)
        texelSize=1.0/textureSize(texture_shadow4,0);
    else if(texIndex==5)
        texelSize=1.0/textureSize(texture_shadow5,0);

    for(int x=-1;x<=1;x++){
        for(int y=-1;y<=1;y++){
            vec2 coord=projCoords.xy+vec2(x,y)*texelSize;
            if(texIndex==0)
                lightViewDepth=texture(texture_shadow0,coord).r;
            else if(texIndex==1)
                lightViewDepth=texture(texture_shadow1,coord).r;            
            else if(texIndex==2)
                lightViewDepth=texture(texture_shadow2,coord).r;
            else if(texIndex==3)
                lightViewDepth=texture(texture_shadow3,coord).r;
            else if(texIndex==4)
                lightViewDepth=texture(texture_shadow4,coord).r;
            else if(texIndex==5)
                lightViewDepth=texture(texture_shadow5,coord).r;
